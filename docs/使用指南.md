...
> 📸 **截图占位（待补充）**：`docs/assets/console-tokens.png`
> 建议画面：令牌卡片——「新建令牌」弹窗，填入 label（如 `Alice 的 VSCode`）、勾选 readonly、自动生成的 value，保存后列表里出现该令牌。
>
> ![令牌卡片](./assets/console-tokens.png)
...
> 📸 **截图占位（待补充）**：`docs/assets/console-transfer.png`
> 建议画面：传送卡片——选中 `webdav` / `sftp` / `direct` 模式，展开对应凭据输入框（仅存于服务端），并显示当前生效模式。
>
> ![传送卡片](./assets/console-transfer.png)
