...
export interface RelayOptions {
  ...
  bindHost?: string;
  /**
   * Token registry required of every plugin session when the relay is bound to a non-loopback host.
   * `null` (the default, loopback-only config) means no token — loopback *is* the boundary. Set via
   * `FIGWRIGHT_TOKEN` / `FIGWRIGHT_TOKENS`; auto-generated by the server in LAN mode when omitted.
   * Checked on both the WebSocket upgrade (as a subprotocol) and the `$hello` params, so a peer that
   * skips either is refused before it can claim a plugin identity. A same-machine peer (loopback
   * address) stays exempt — loopback is always trusted, even in LAN mode.
   */
...
export class Relay {
  ...
  constructor(opts: RelayOptions) {
    ...
    this.wss = new WebSocketServer({
      ...
      verifyClient: ({ req }, done) => {
        ...
        // LAN mode: the loopback boundary is gone for *remote* peers, so a token is the only thing
        // standing between the relay and any other machine on the network. The plugin offers the
        // secret as a WebSocket subprotocol; a remote peer that names none of our valid tokens (a
        // stray LAN client, a probe) is refused here, before it can even send $hello. A same-machine
        // peer (loopback address) is the user and stays exempt — loopback is always trusted, even in
        // LAN mode. Loopback-mode configs set no registry and skip this entirely.
        if (this.opts.tokens !== null && !isLoopbackAddress(req.socket.remoteAddress)) {
          const offered = (req.headers['sec-websocket-protocol'] ?? '')
            .split(',')
            .map(s => s.trim());
          const matched = offered.find(p => this.opts.tokens!.has(p));
          if (matched === undefined) {
            this.opts.log(
              `[relay] refused WebSocket upgrade without a valid token for host ${req.headers.host ?? '(none)'}`,
            );
            done(false, 403, 'Forbidden');
            return;
          }
  ...
  private handleConnection(socket: WebSocket): void {
    ...
    socket.on('close', () => {
      ...
      if (session !== undefined) {
        ...
        const peerType = isLoopbackAddress(peerIp) ? 'local-plugin' : 'remote-plugin';
  ...
  private handleHello(socket: WebSocket, env: Envelope): Session | null {
    ...
    // LAN mode: the WebSocket upgrade already required a valid token as a subprotocol (for remote
    // peers; loopback peers are exempt), but a peer that replays a captured upgrade or reaches us
    // through a path that skipped it must still be turned away at the handshake. Re-checking here —
    // where we can return a clear error to the panel — is defence in depth, and the only gate for any
    // transport that doesn't carry subprotocols. Same-machine loopback peers stay exempt.
    const peerIp = remoteIpOf(socket);
    const isLocal = isLoopbackAddress(peerIp);
    if (this.opts.tokens !== null && !isLocal && !this.opts.tokens.has(parsed.data.token)) {
      const message =
      ...
      this.sendError(socket, env, ErrorCode.InvalidParams, message, { reason: 'token' });
      return null;
    }
    const peerType = isLocal ? 'local-plugin' : 'remote-plugin';
    const peerLabel = this.opts.tokens?.match(parsed.data.token, false)?.label ?? 'plugin';
    this.opts.log(`[peer] connect type=${peerType} ip=${peerIp} token=${peerLabel}`);
