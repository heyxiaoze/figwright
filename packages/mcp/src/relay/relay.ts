 ...
 * local control channel; remote machines reach the server through /mcp (token-gated), never through
 * this relay, so loopback is the entire trust boundary and no token is involved for local plugins.
 */
export const isRelayUpgradeAllowed = (req: IncomingMessage): boolean =>
  isLoopbackAddress((req.socket as { remoteAddress?: string } | null)?.remoteAddress);

/**
 * Resolve the peer's IP from a `ws` WebSocket. `WebSocket.remoteAddress` is usually set once the
 * connection is established, but in some upgrade timings it reads undefined — fall back to the
 * underlying socket so loopback peers are still recognised as local (and labelled correctly) rather
...
export interface RelayOptions {
  ...
  server: HttpServer;
  /** The host the relay bound to. Drives the conditional host gate in local-access (loopback stays
   * strict; a LAN bind widens the allow-list to this machine's own interfaces). Defaults to
   * loopback when omitted, so existing callers/tests need not pass it. */
  bindHost?: string;
  log?: (msg: string) => void;
...
export class Relay {
  ...
  constructor(opts: RelayOptions) {
    ...
    this.wss = new WebSocketServer({
      ...
      verifyClient: ({ req }, done) => {
        if (!isAllowedHost(req.headers.host, this.opts.bindHost)) {
          this.opts.log(
        ...
        // The relay is the plugin's local control channel and is intentionally loopback-only: only a
        // plugin on this same machine (Figma on the same Mac, addressing 127.0.0.1/::1) may open it.
        // Every other machine on the network is refused at the upgrade, before it can claim a plugin
        // identity. Remote MCP clients connect through /mcp (HTTP, token-gated) — a different
        // endpoint on the same socket — so this gate never touches them. Loopback IS the trust
        // boundary; no token is involved for local plugins.
        if (!isRelayUpgradeAllowed(req)) {
          this.opts.log(
            `[relay] refused WebSocket upgrade from non-loopback ${req.socket.remoteAddress ?? '(unknown)'} — relay is local-only`,
          );
          done(false, 403, 'Forbidden');
          return;
        }
  ...
  private handleConnection(socket: WebSocket): void {
    ...
    socket.on('close', () => {
      clearTimeout(helloTimeout);
      if (session !== undefined) {
        const peerIp = remoteIpOf(socket);
        const peerType = 'local-plugin';
        this.opts.log(`[peer] disconnect type=${peerType} ip=${peerIp}`);
        this.opts.log(
  ...
  private handleHello(socket: WebSocket, env: Envelope): Session | null {
    ...
    // The relay is loopback-only (enforced at the upgrade), so every accepted plugin is a
    // same-machine local plugin. No token is involved for local plugins.
    const peerIp = remoteIpOf(socket);
    const peerType = 'local-plugin';
    this.opts.log(`[peer] connect type=${peerType} ip=${peerIp} token=-`);
