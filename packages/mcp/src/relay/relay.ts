...
export interface RelayOptions {
  ...
  bindHost?: string;
  /**
   * Shared secret required of every plugin session when the relay is bound to a non-loopback host.
   * Absent (the default, loopback-only config) means no token — loopback *is* the boundary. Set via
   * `FIGWRIGHT_TOKEN`; auto-generated by the server in LAN mode when omitted. Checked on both the
   * WebSocket upgrade (as a subprotocol) and the `$hello` params, so a peer that skips either is
   * refused before it can claim a plugin identity.
   */
      ...
      verifyClient: ({ req }, done) => {
        ...
        if (this.opts.token !== undefined) {
          const offered = (req.headers['sec-websocket-protocol'] ?? '')
            .split(',')
            .map(s => s.trim());
          if (!offered.includes(this.opts.token)) {
            this.opts.log(
              `[relay] refused WebSocket upgrade without token for host ${req.headers.host ?? '(none)'}`,
            );
            done(false, 403, 'Forbidden');
            return;
          }
